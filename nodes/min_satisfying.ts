 ...
 * minSatisfying(["1.2.3", "1.3.0", "2.0.0"], "^1.0.0") -> "1.2.3". Entries
 * of `versions` that are not themselves valid SemVer versions are silently
 * skipped, mirroring MaxSatisfying.
 *
 * `ok` is false when the range is invalid, OR when no entry in `versions`
 * satisfies it — `error` is set only for the former.
 *
 * @param ax - Platform context: ax.log for logging, ax.secrets for secrets.
 */
export function minSatisfying(ax: AxiomContext, input: SemverListRangeRequest): SemverVersionResult {
  ...
  try {
    ...
    const result = semver.minSatisfying(versions, range, opts);
    if (result === null) {
      out.setOk(false);
      return out;
    }
    ...
    out.setOk(false);
    out.setError(errorMessage(e, 'finding min satisfying version'));
    return out;
  }
}
